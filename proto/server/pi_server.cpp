...
// TODO(antonin): uncomment when client code has been updated
...
class DeviceState {
 public:
  ...
  void send_packet_in(p4::PacketIn *packet) const {
    std::lock_guard<std::mutex> lock(m);
    // TODO(antonin): use this code when client code has been updated
  ...
  Status update_connection(Connection *connection,
    ...
    auto is_master = (p.first == connections.begin());
    auto master_changed =
        (is_master && !was_master) || (was_master && !is_master);
    if (master_changed)
      notify_all();
    else
      notify_one(connection);
        ...
        case p4::StreamMessageRequest::kArbitration:
          {
            ...
            // TODO(antonin): uncomment when client code is updated
            // if (election_id == 0) {
